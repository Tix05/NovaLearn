...
class ExamenController extends AbstractController
{
    ...
    public function createExamen(Request $request): JsonResponse
    {
        ...
        $duration = isset($data['duree']) && is_numeric($data['duree']) && $data['duree'] > 0 ? (int)$data['duree'] : 3600;
        ...
        $examen = new Examen();
        $examen->setTitre($data['titre'] ?? 'Examen');
    ...
    /**
     * @Route("/{id}/publish", name="api_examen_publish", methods={"GET", "POST"})
     */
    public function publishExamen(int $id, Request $request): JsonResponse
    {
        $this->logger->info('Requête reçue pour /api/examen/{id}/publish', ['id' => $id, 'method' => $request->getMethod()]);

        $user = $this->security->getUser();
        if (!$user || !in_array('ROLE_ADMIN', $user->getRoles())) {
            $this->logger->error('Accès non autorisé pour publication', ['user' => $user ? $user->getEmail() : 'anonyme']);
            return $this->json(['message' => 'Accès non autorisé'], 403);
        }
        ...
        $data = $request->getContent() ? json_decode($request->getContent(), true) : $request->query->all();
        ...
        if (!$dateDebut || !$dateFin) {
            $this->logger->warning('Dates de début ou de fin manquantes');
            return $this->json(['message' => 'Dates de début et de fin requises'], 400);
        }

        if ($dateDebut >= $dateFin) {
            $this->logger->warning('La date de fin doit être postérieure à la date de début');
            return $this->json(['message' => 'La date de fin doit être postérieure à la date de début'], 400);
        }

        if ($isPreview || $request->getMethod() === 'GET') {
            $previewData = [
                'id' => $examen->getId(),
                'titre' => $examen->getTitre(),
                'description' => $examen->getDescription() ?? 'N/A',
                'dateDebut' => $dateDebut->format('Y-m-d H:i'),
                'dateExpiration' => $dateFin->format('Y-m-d H:i'),
                ...
                'auteur' => $examen->getAuteur()->getName(),
                'fichier' => $examen->getFichier() ? '/uploads/examens/' . $examen->getFichier() : null,
            ];

            $this->logger->info('Prévisualisation de l\'examen générée', ['examen_id' => $id]);
            return $this->json([
                'message' => 'Prévisualisation de l\'examen',
                'preview' => $previewData,
            ], 200);
        }
        ...
        try {
            ...
            $agenda->setDescription($examen->getDescription() ?? 'Examen pour ' . $examen->getEc()->getName());
            $agenda->setType('EXAMEN');
            $agenda->setNomAuteur($examen->getAuteur()->getName());
            $agenda->setMention($examen->getEc()->getUe()->getMention());
            $agenda->setNiveau($examen->getEc()->getUe()->getSemestre()->getNiveau());
            $agenda->setCreatedAt(new \DateTime());
            $agenda->setUpdatedAt(new \DateTime());

            $this->entityManager->persist($agenda);

            $notification = new Notification();
            $notification->setTitre('Nouvel examen publié : ' . $examen->getTitre());
            $notification->setContenu('Un nouvel examen est disponible pour ' . $examen->getEc()->getName() . ' du ' . $dateDebut->format('d/m/Y H:i') . ' au ' . $dateFin->format('d/m/Y H:i') . '.');
            $notification->setType('ANNONCE_ADMIN');
            $notification->setLien('/examen/' . $examen->getId());
            $notification->setDateCreation(new \DateTime());
            $notification->setCreatedAt(new \DateTime());
            $notification->setUpdatedAt(new \DateTime());
            $this->entityManager->persist($notification);

            $notificationGroupe = new NotificationGroupe();
            $notificationGroupe->setNotification($notification);
            $notificationGroupe->setMention($examen->getEc()->getUe()->getMention());
            $notificationGroupe->setNiveau($examen->getEc()->getUe()->getSemestre()->getNiveau());
            $notificationGroupe->setCreatedAt(new \DateTime());
            $this->entityManager->persist($notificationGroupe);
            ...
            $this->logger->info('Examen publié et ajouté à l\'agenda', ['examen_id' => $id, 'agenda_id' => $agenda->getId()]);
            return $this->json([
            ...
            $this->logger->error('Erreur lors de la publication: ' . $e->getMessage(), ['exception' => $e]);
    ...
    public function submitExamen(int $id, Request $request): JsonResponse
    {
        ...
        $reportFileName = $this->generateCorrectionPdf($examen, $correction);
    ...
    public function getTeacherExams(): JsonResponse
    {
        ...
        try {
            ...
            $examens = $this->examenRepository->findBy(['statut' => 'soumis']);
    ...
    private function generateCorrectionPdf(Examen $examen, array $correction): string
    {
        $pdf = new TCPDF();
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Système de Correction');
        $pdf->SetTitle('Rapport de Correction');
        ...
        $pdf->Cell(0, 10, sprintf('Note Totale: %d/20', $correction['total_score']), 0, 1);
        $pdf->Ln(5);
        ...
        foreach ($correction['questions'] as $qId => $qData) {
            ...
            $pdf->SetFont('helvetica', '', 12);
            $pdf->MultiCell(0, 10, 'Feedback: ' . $qData['feedback'], 0, 'L');
            $pdf->Cell(0, 10, sprintf('Score: %s/%s', $qData['score'], $qData['points'] ?? 1), 0, 1);
            $pdf->Ln(5);
        }
    ...
    private function generateExamPdf(Examen $examen, array $questions): string
    {
        ...
        // Marquer comme temporaire pour exclusion de la visualisation
        $pdf->SetHeaderData('', 0, '', 'Temporary: DoNotDisplay');
        $pdf->AddPage();
